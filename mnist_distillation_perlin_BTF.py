...
def generate_perlin_noise_2d(shape, res):
    def f(t):
        ...
        return np.random.laplace(0,1,t.shape)
...
# build the function in python into Tensorflow
perlin_fn = tf.py_function(func=generate_perlin_noise_2d, inp=[tf.Variable([28., 28.]), tf.Variable([28., 28.])], Tout=tf.float32)

# mapping the funtion before prefetch
#MNIST_imgs = tf.map_fn(lambda x: tf.reshape(perlin_fn, [-1]), MNIST_imgs)

# use TF.dataset to handle minst
MNIST_dataset = tf.data.Dataset.from_tensor_slices({'imgs':MNIST_imgs, 'labs':MNIST_labels})
MNIST_dataset = MNIST_dataset.apply(tf.contrib.data.shuffle_and_repeat(buffer_size=100))
MNIST_dataset = MNIST_dataset.prefetch(buffer_size=50000) # prefech
...
# mapping the function after prefetch
#noise = tf.map_fn(lambda x: tf.reshape(perlin_fn, [-1]), MNIST_dataset_fetch['imgs'])
#logits_s =  conv_net(noise, num_classes, 0, reuse=False, is_training=False)
#logits_q = qconv_net(noise, num_classes, 0, reuse=False, is_training=False)
...
#optimizer = tf.train.RMSPropOptimizer(learning_rate=1E-4, decay=.9, momentum=.0, centered=True)
#optimizer = tf.contrib.opt.AdamWOptimizer(1E-4, learning_rate=1E-6)
optimizer = tf.train.MomentumOptimizer(learning_rate=6E-5, momentum=.9)
...
while(1):
    ...
    if training_step % 1000 == 0:
        ...
        #noise_o = np.random.gumbel(0, 1.5, [batch_size * 1000, 784]) 
        noise_o = np.zeros( [batch_size * 1000, 784]) #only for dummy input
