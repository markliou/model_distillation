...
def noise_gen(x, reuse=False):
    with tf.device('/device:GPU:0'):    
        with tf.variable_scope('noise_gen', reuse=reuse):
            ...
            for i in range(8):
                fc = tf.keras.layers.Dense(512, activation=tf.nn.tanh, kernel_initializer='glorot_uniform')(fc)
...
logits_test_check = conv_net(MNIST_imgs, num_classes, dropout=0, reuse=True, is_training=False)
...
##check
pred_classes_c = tf.cast(tf.argmax(logits_test_check, axis=1), tf.float32)
pred_probas_c = tf.nn.softmax(logits_test_check)
acc_op_c = tf.reduce_mean(tf.cast(tf.equal(tf.reshape(MNIST_labels,tf.shape(pred_classes_c)), pred_classes_c), tf.float32)) # accuracy
...
#noise_train_op = noise_opt.minimize(tf.exp( -loss_op * 5e-2) * 1e2 , var_list=noise_gen_var)
noise_train_op = noise_opt.minimize(noise_gen_loss - loss_op, var_list=noise_gen_var)
...
ad_labs = np.random.randint(10, size=[batch_size * 1000])
sess.run(MNIST_dataset_iter.initializer, feed_dict={MNIST_imgs: np.random.gumbel(0, .8, [batch_size * 1000, 784]),
...
while(1):
    ...
    if training_step % 1000 == 0:
        ...
        # test
        acc, acc_c = sess.run([acc_op, acc_op_c], feed_dict={MNIST_imgs: np.array(mnist.test.images),
        ...
        #print("Testing Accuracy:{} highest:({}) nloss:{}".format(acc , highest_acc , np.exp(-closs * 5e-2)))
        print("Testing Accuracy:{}({}) highest:({}) nloss:{}".format(acc , acc_c, highest_acc , np.exp(-closs * 5e-2)))
