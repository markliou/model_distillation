...
#dis_s = tf.distributions.Categorical(logits=(tf.random.normal([1])[0]+1)*logits_s)
...
#optimizer = tf.train.RMSPropOptimizer(learning_rate=1E-5, decay=.9, momentum=.0)
...
# training
sess.run(MNIST_dataset_iter.initializer, feed_dict={MNIST_imgs: np.random.gumbel(0, .2, [batch_size * 1000, 784]),
...
while(1):
    ...
    if training_step % 1000 == 0:
        ...
        noise_o = np.random.gumbel(0. , 1., [batch_size * 1000, 28 * 28])
